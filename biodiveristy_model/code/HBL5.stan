...
int<lower=0> J;      //number of characteristics for individuals
array[T] int<lower=0> ids; //group id
array[T] int y;       //the choices
array[M,T] row_vector[K] x;  // the attributes matrix
array[N] row_vector[J] Z;    // the sociodemographic characteristics including distance to preferred sites
...
matrix[J, K-2] demog_beta_coeffs;  // covariate effects on beta means
vector[J] demog_theta_coeffs;      // covariate effects on theta mean
}
...
{   array[T] vector[M] mu;
    ...
    real alpha_std;
    real theta_std;
    array[N] vector[K-2] beta_mu_ind;
    array[N] real theta_mu_ind;
    ...
    for(n in 1:N)
       {
        beta_mu_ind[n] = beta_mu + to_vector(Z[n] * demog_beta_coeffs);
        theta_mu_ind[n] = theta_mu + dot_product(Z[n], demog_theta_coeffs);
...
to_vector(demog_beta_coeffs) ~ normal(0, 1);
demog_theta_coeffs ~ normal(0, 1);
...
{  alpha[n]  ~ normal(alpha_mu, alpha_std);
   theta[n]  ~ normal(theta_mu_ind[n], theta_std);

   for(k in 1:K-2) {beta[n,k] ~ normal(beta_mu_ind[n,k], beta_std[k]);} 
